...
const mapToExcelRow = (section, d, branchName) => {
  ...
  switch (section) {
    case "switch": return {...base,"Asset Name":d?.asset_name||"","Model":d?.model||"","Type":d?.type||"","Brand":d?.brand||"","Location":d?.location||"","Port":d?.port||"","Assigned User":d?.assigned_user||"",Status:d?.status||"Active"};
    ...
    case "extra_monitor": return {...base,"Monitor Brand":d?.monitor_brand||"","Monitor Size":d?.monitor_size||"","Monitor Location":d?.monitor_location||"","Monitor Status":d?.monitor_status||"","System Model":d?.system_model||"","Assigned User":d?.assigned_user||"",Status:d?.status||d?.monitor_status||"Active"};
    case "server": return {...base,Brand:d?.brand||"","IP Address":d?.ip_address||"",Location:d?.location||"","Model No":d?.model_no||"","Purchase Date":formatDateForExcel(d?.purchase_date),Vendor:d?.vendor||d?.vendor_name||"",Specification:d?.specification||"",Storage:d?.storage||"",Memory:d?.memory||"","Window Server Version":d?.windows_server_version||"",Virtualization:d?.virtualization||"",Status:d?.status||"Active"};
    case "firewall_router": return {...base,Brand:d?.brand||"",Model:d?.model||"","Purchase Date":formatDateForExcel(d?.purchase_date),Vendor:d?.vendor||d?.vendor_name||"","Liscence-expiry":formatDateForExcel(d?.license_expiry||d?.expiry_date),Status:d?.status||"Active"};
    ...
    case "scanner": return {...base,Name:d.scanner_name||"",Model:d.scanner_model||"","Assigned User":d.assigned_user||"",Location:d.location||"",Status:d.status||"Active"};
    ...
    case "cctv": return {...base,Brand:d?.cctv_brand||"","NVR IP":d?.cctv_nvr_ip||"","Record Days":d?.cctv_record_days||"",Capacity:d?.capacity||"",Channel:d?.channel??"",Vendor:d?.vendor||"","Purchase Date":formatDateForExcel(d?.purchase_date),Status:d?.status||"Active"};
...
const SECTION_FIELD_MAP = {
  ...
  extra_monitor:{Section:(d,r)=>r?.section,Branch:(d,r)=>r?.branch,"Asset Code":(d)=>d?.assetId,"Sub-Cat Code":(d)=>d?.sub_category_code,"Monitor Brand":(d)=>d?.monitor_brand,"Monitor Size":(d)=>d?.monitor_size,"Monitor Location":(d)=>d?.monitor_location,"Monitor Status":(d)=>d?.monitor_status,"System Model":(d)=>d?.system_model,"Assigned User":(d)=>d?.assigned_user,Status:(d)=>d?.status||d?.monitor_status,Remarks:(d)=>d?.remarks},
  switch:{Section:(d,r)=>r?.section,Branch:(d,r)=>r?.branch,"Asset Code":(d)=>d?.assetId,"Sub-Cat Code":(d)=>d?.sub_category_code,"Asset Name":(d)=>d?.asset_name,"Model":(d)=>d?.model,"Type":(d)=>d?.type,"Brand":(d)=>d?.brand,"Location":(d)=>d?.location,"Port":(d)=>d?.port,"Assigned User":(d)=>d?.assigned_user,Status:(d)=>d?.status,Remarks:(d)=>d?.remarks},
  ...
  scanner:{Section:(d,r)=>r?.section,Branch:(d,r)=>r?.branch,"Asset Code":(d)=>d?.assetId,"Sub-Cat Code":(d)=>d?.sub_category_code,Name:(d)=>d?.scanner_name,Model:(d)=>d?.scanner_model,"Assigned User":(d)=>d?.assigned_user,Location:(d)=>d?.location,Status:(d)=>d?.status,Remarks:(d)=>d?.remarks},
...
function toReportRows(branches, subCatMap, groupMap) {
  ...
  for (const b of branches||[]) {
    ...
    const pushDevice=(section,row)=>{
      ...
      pushRow(section,row,{subCategoryCode:row?.sub_category_code||"",name:deviceName,brand:getBrand(row)||"",model:getModel(row)||"",purchaseYear,status:
        row?.monitor_status ||
        row?.printer_status ||
